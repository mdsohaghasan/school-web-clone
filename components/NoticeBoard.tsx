  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
          ...
          <li key={notice.id}>
            <Link href={notice.link} className="block hover:bg-gray-100 p-2 rounded transition duration-300">
              <div className="flex items-start">
                <FaBullhorn className="text-foreground mt-1 mr-2 flex-shrink-0" />
                <div>
                  <h4 className="font-semibold text-high font-hind-siliguri">{notice.title}</h4>
                  <p className="text-sm text-mid font-hind-siliguri">{notice.date}</p>
                </div>
              </div>
            </Link>
          </li>
      ...
      <div className="mt-4 text-center">
        <Link href="/notices" className="text-foreground hover:underline font-hind-siliguri">
          সকল নোটিশ দেখুন
        </Link>
      </div>
    </div>
